...
import { getById, updateProperty } from "@/services/property.service";
...
const defaultModel: IFormEditProperty = {
  type: "",
  name: "",
  description: "",
  street: "",
...
const AddProperty: React.FC = () => {
  const [model, setModel] = useState<IFormEditProperty>({ ...defaultModel });
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        ...
        const amenities = { ...defaultModel.amenities };
        property.amenities.forEach(
          (item: keyof IAmenities) => (amenities[item] = true)
        );
        const modelData: IFormEditProperty = {
          type: property.type,
          name: property.name,
          ...
          amenities,
          weekly_rate: property.rates.weekly ?? "",
          monthly_rate: property.rates.monthly ?? "",
          nightly_rate: property.rates.nightly ?? "",
          seller_name: property.seller_info.name,
          seller_email: property.seller_info.email,
          seller_phone: property.seller_info.phone,
          // images:
        };
        setModel(modelData);
      } catch (error) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    try {
      const res = await updateProperty(id as string, model);
      if (!res) {
